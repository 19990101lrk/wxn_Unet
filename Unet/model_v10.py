...
from SE_net import squeeze_excitation_layer
...
def res_block(in_put, n_kernels):
    ...
    conv_SE = squeeze_excitation_layer(conv, n_kernels, 1)
    scale = Multiply()([conv, conv_SE])
...
def unet(pretrained_weights=None, input_size=(IMG_SIZE, IMG_SIZE, 3), num_class=12):
    ...
    conv1_SE = squeeze_excitation_layer(conv1, 64, 1)
    scale = Multiply()([conv1, conv1_SE])

    conv1_shortcut = Conv2D(64, 3, activation='relu', padding='same', kernel_initializer='he_normal')(inputs)
    # conv1_shortcut = LeakyReLU(alpha=0.5)(conv1_shortcut)
    conv1_shortcut = BatchNormalization(axis=3)(conv1_shortcut)
    conv1 = Add()([scale, conv1_shortcut])
