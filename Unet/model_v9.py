...
def res_block(in_put, n_kernels):
    ...
    conv = Activation('relu')(conv)
    conv = AtrousConvolution2D(n_kernels, 3, padding='same', kernel_initializer='he_normal')(conv)
...
def unet(pretrained_weights=None, input_size=(IMG_SIZE, IMG_SIZE, 3), num_class=12):
    ...
    # conv1_down = AtrousConvolution2D(64, 3, rate=2, activation='relu', padding='same', kernel_initializer='he_normal')(conv1)
    ...
    # conv1_1 = ASPP(conv1, 64)
    # conv5 = res_block(pool4, n_kernels=1024)
    conv5 = ASPP(pool4, 1024)
